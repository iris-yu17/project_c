...
// 新增訂單資料到「我的訂單」資料表
    router.post('/my-order', async (req, res)=>{
        ...
        const rebuild =req.body.myOrderDetailArray.map((item)=>[item.order_sid,item.product_sid,item.product_amount,item.product_name,item.product_price ])
        // console.log(rebuild)
        // const [{affectedRows, insertId}] = await db.query(sql2, [rebuild]);
        // res.json({
        //     success: !!affectedRows,
        //     affectedRows,
        //     insertId,
